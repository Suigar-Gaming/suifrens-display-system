...
import { BullsharkExpression } from "../../types.js";

type RightArmProps = SVGProps<SVGPathElement> & {
  expression: BullsharkExpression;
};

export function RightArm({ expression, ...armProps }: RightArmProps) {
  const baseTransform =
    expression === "ourah" || expression === "showing"
  ...
  const transform = useAnimatedPartTransform("rightArm", baseTransform);
